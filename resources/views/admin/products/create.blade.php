...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-6 sm:p-8">
        <form id="product_form" method="POST" action="{{ route('admin.products.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <!-- Hidden input storing the URL for form submission -->
                ...
                <div id="product_image_dropzone"
                    ...
                    <div class="flex items-center space-x-2.5 overflow-hidden">
                        ...
                        <span id="product_image_placeholder" class="text-sm {{ old('image_url') ? 'text-slate-700 font-medium' : 'text-slate-400 group-hover:text-slate-500' }} transition select-none truncate">
                            {{ old('image_url') ? 'Uploaded image (' . basename(old('image_url')) . ')' : 'Click to upload image to Supabase Storage' }}
                        </span>
                    </div>
                ...
                <!-- Image Preview (appears when image is present) -->
                <div id="product_image_preview_box" class="mt-3 {{ old('image_url') ? '' : 'hidden' }}">
            ...
            <div class="pt-4 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <label class="flex items-center space-x-2.5 cursor-pointer">
                    <input type="checkbox" name="is_digital" id="is_digital" value="1" {{ old('is_digital') ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300" onchange="toggleDigitalSection()">
            ...
            <div id="digital_fields" class="p-4 bg-purple-50/70 rounded-xl border border-purple-200 space-y-3 {{ old('is_digital') ? '' : 'hidden' }}">
                <div>
                    ...
                    <input type="hidden" name="digital_file_path" id="digital_file_path" value="{{ old('digital_file_path') }}">
                    ...
                    <div id="digital_file_dropzone"
                        ...
                        <div class="flex items-center space-x-2.5 overflow-hidden">
                            ...
                            <!-- Placeholder / Filename text -->
                            <span id="digital_file_placeholder" class="text-sm {{ old('digital_file_path') ? 'text-slate-700 font-medium' : 'text-slate-400 group-hover:text-slate-500' }} transition select-none truncate">
                                {{ old('digital_file_path') ? 'Uploaded: ' . basename(old('digital_file_path')) : 'Click to upload digital file' }}
                            </span>
                        </div>
                    ...
                    <div id="digital_file_description" class="mt-2 {{ old('digital_file_path') ? 'flex' : 'hidden' }} items-center justify-between text-xs text-purple-700 bg-purple-100/70 px-3 py-1.5 rounded-md border border-purple-200">
                        <div class="flex items-center space-x-1.5 overflow-hidden">
                            ...
                            <span id="digital_file_display_name" class="font-mono truncate">
                                Current file: {{ old('digital_file_path') ? basename(old('digital_file_path')) : '' }}
                            </span>
                        </div>
                        <button type="button"
...
<script>
    ...
    productForm.addEventListener('submit', function(e) {
        ...
        if (parseInt(priceInput.value) === 0) {
            if (!confirm('Price is set to 0. Are you sure you want to create a free product?')) {
                e.preventDefault();
                return false;
            }
        }
